...
int main(int, char**)
{
    ...
    ::RegisterClassExW(&wc);
    HWND hwnd = ::CreateWindowW(wc.lpszClassName, L"Chatroom", WS_OVERLAPPEDWINDOW, 100, 100, (int)(1280 * main_scale), (int)(800 * main_scale), nullptr, nullptr, wc.hInstance, nullptr);
    ...
    while (!done)
    {
        ...
        else
        {
            ...
            if (ImGui::InputText("##GlobalInput", global_input, IM_ARRAYSIZE(global_input), ImGuiInputTextFlags_EnterReturnsTrue)) {
            ...
            for (int i = 0; i < open_private_chats.size(); i++)
            {
                ...
                if (ImGui::Begin(window_name.c_str(), &chat.is_open))
                {
                    ...
                    std::string input_label = "##Input" + chat.target_username; // Unique ID for input box

                    if (ImGui::InputText(input_label.c_str(), chat.input_buf, 256, ImGuiInputTextFlags_EnterReturnsTrue)) {
